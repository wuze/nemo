...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		case WM_PAINT:
			...
			step=width*1.0/POINT_NUM;
			rac=2.0*PI/POINT_NUM;
			
			hpen=CreatePen(PS_SOLID,1,RGB(255,0,0));
			hOldpen=(HPEN)SelectObject(hdc,hpen);

			TextOut(hdc,width/2,height/2+10,"PI/2",strlen("PI/2"));
			...
			{
				pt[i].x=int(step*i);
				pt[i].y=int(height/2*(1-sin(rac*i)));
	
				//SetPixel(hdc,pt[i].x,pt[i].y,RGB((12*i)%255,(23*i)%255,(100*i)%255));
			}

			hpen=CreatePen(PS_SOLID,2,RGB(123,134,200));
			hOldpen = (HPEN)SelectObject(hdc,hpen);
